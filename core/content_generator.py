...
class ContentGenerator:
        ...
        return [
            ...
            },
            {
                ...
                "description": (
                    ...
                    "使用小红书MCP工具，将格式化后的内容进行发布：\n"
                    "   - title作为动态标题\n"
                    "   - content作为正文内容\n"
                    "   - images作为配图（3-4张高质量图片）\n"
                    "   - tags作为话题标签\n"
                    "**注意**: 前面的步骤经过了详细的信息收集了，只需要将内容进行发布即可，不需要这一步做额外的查询工作，发布成功就不要再进行工具调用了"
                ),
    ...
    async def execute_step(self, step: Dict[str, Any], available_tools: List[Tool],
        ...
        # 构建系统提示词（参考xhs_context_generate.py）
        system_prompt = f"""你是一个专业的小红书内容创作AI助手，擅长收集信息、分析内容并创作吸引人的社交媒体文案。

                【核心任务】
                研究主题：「{user_topic}」
                最终目标：创作一篇高质量的小红书内容并成功发布
